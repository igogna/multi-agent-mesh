...
- **Mandatory human plan-approval gate**: `run.py` always stops and prints the Plan for a `y`/`N` approval before any code is generated — there is no path that skips this. Rejecting re-plans with your feedback, up to `max_plan_review_rounds` (default 3), then stops the run entirely rather than falling through to code generation.
- `code_generator` produces full file rewrites against that plan; `test_generator` writes tests in whatever framework matches the repo's detected language, incorporating feedback from a prior failed attempt when retrying.
- **Multi-language sandbox** (`tools/language_config.py` + `sandbox_tools`): the target repo's language is auto-detected from its marker files (`pyproject.toml`→Python, `package.json`→Node/TS, `go.mod`→Go, `build.gradle(.kts)`/`pom.xml`→Java/Kotlin, an `AndroidManifest.xml` anywhere→Android/Kotlin), and tests/lint run inside the matching Docker image (`sandbox/<language>/Dockerfile`) — generated code never runs on the host. Android support covers Gradle-wrapper unit tests (`testDebugUnitTest`) only — no emulator/instrumented tests, same limitation as iOS Simulator tests. Failures feed back into another code-gen round, up to `max_iterations`.
